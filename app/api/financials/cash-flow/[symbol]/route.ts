...
export async function GET(request: Request, context: { params: { symbol: string } }) {
    ...
    console.log(`GET request received for cash flow statement of ${symbol}.`);
    ...
    try {
        ...
        // Retrieve cash flow statements with pagination
        const { data: cashFlows, error, count } = await supabase
            .from("cash_flow")
            .select(
                `
                *
        ...
        if (!cashFlows || cashFlows.length === 0) {
            ...
            return NextResponse.json(
                {
                    error: `No cash flow statements found for the stock symbol '${symbol}'. Please verify the entered symbol.`,
                },
                { status: 404 }
            );
        }

        // Record successful usage in usage_logs
        await logUsage(
            supabase,
            apiKeyValidation.userId,
            apiKeyValidation.keyId,
        ...
        return NextResponse.json({
            ...
            message: `Cash flow statement for symbol '${symbol}' retrieved successfully. Displaying page ${validatedPage + 1} of ${Math.ceil((count || 0) / validatedLimit)} (Total: ${count || 0} entries).`,
        });
    } catch (error: any) {
        ...
        return NextResponse.json(
            { error: `An error occurred while retrieving the cash flow statement for symbol '${symbol}'. Please try again later. Details: ${error.message || "N/A"}` },
            { status: 500 }
        );
    }
}